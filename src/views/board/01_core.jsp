...
<%-- 
	...
	uri 는 오타가 있으면 절대 안됨!! (연동이 제대로 되지 않음)
--%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<!-- session.setAttribute("result", (int)pageContext.getAttribute("num1") + (int)request.getAttribute("num2")); 와 같음 -->
	...
	request 로 부터 삭제 후 result : ${ resutl } <br><br>
	...
	<pre>
		* 변수 출력(&lt;c:out value="출력하고자하는값" default="기본값(생략가능)" excapeXml="true/false"&gt;)
		
		- 데이터를 출력하고자 할 때 사용하는 태그
		- default : value 에 출력하고자 하는 값이 없을 경우 기본값으로 대체해서 출력할 내용물을 기술하는 속성 (생략 가능)
		- escapeXml : html 태그를 태그로써 해석해서 출력할지에 대한 여부 (생략 가능, 생략 시 true 가 기본값 => 태그로 해석 안됨)
	</pre>
	...
	<!-- pageContext.setAtrribute("outTest", "<b>출력테스트</b>"); 와 같음 -->
	...
	<pre>
		...
		- 조건식은 test 라는 속성에 작성 (단, EL 구문으로 조건을 작성해야 함!!)
	</pre>
	
	<%-- 
	<%
		int num1 = (int)pageContext.getAttribute("num1");
		int num2 = (int)pageContext.getAttribute("num2");
	%>
	...
	<!-- 기존 방식 예시 : 10과 3을 이용해서 대소비교하는 조건식 제시 -->
	<% if(10 > 3) { %>
		<b>10이 3보다 큽니다.</b>
	<% } %>
	...
	<c:forEach var="i" begin="1" end="6" step="i">
	...
	<!-- 응용 -->
	<%
		// Servlet 으로 부터 다음과 같은 데이터를 넘겨받았다는 가정 하에
		ArrayList<Person> list = new ArrayList<>();
		list.add(new Person("홍길동", 20, "남자"));
		list.add(new Person("김말똥", 30, "여자"));
	...
	<!-- 기존 방식 -->
	<a href="list.do?currentPage=1&num=2&keyword=안녕">기존방식</a>
